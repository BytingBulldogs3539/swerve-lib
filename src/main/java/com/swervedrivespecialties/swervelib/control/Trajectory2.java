...
import com.swervedrivespecialties.swervelib.math.MathUtils;
...
public class Trajectory2 {
    ...
    public Trajectory2(Path path, TrajectoryConstraint[] trajectoryConstraints, double sampleDistance) {
    ...
    public Trajectory2(Path path, TrajectoryConstraint[] trajectoryConstraints, double sampleDistance,
        ...
        while (distance < seg.getLength()) {
            Path.State startingState = path.calculate(distance+totalDistance);

            double profileLength = sampleDistance;
            if (distance + sampleDistance > seg.getLength()+totalDistance) {
                profileLength = seg.getLength() - distance;
            }
    ...
    class ConstrainedPathState {
        ...
        public ConstrainedPathState(Path.State pathState, double length, double startingVelocity, double endingVelocity,
                double acceleration) {
        ...
        public State calculate(double time) {
            ...
            double distance = 0.5 * acceleration * Math.pow(time, 2.0) + startingVelocity * time
